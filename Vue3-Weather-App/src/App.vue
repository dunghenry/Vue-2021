<template>
  <div
    id="app"
    :class="
      typeof data.main != 'undefined' && data.main.temp > 16 ? 'warm' : ''
    "
  >
    <main>
      ...
      <div class="weather-wrap" v-if="typeof data.main != 'undefined'">
        ...
        <div class="image">
          <img
            v-bind:src="`http://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`"
            alt="Icon"
          />
        </div>
        ...
        <div class="additional-section">
          <div class="row">
            <div class="item">
              <label>MT Mọc</label>
              <div class="sunrise"> {{moment.unix(data.sys.sunrise).format('H:mm')}}</div>
            </div>
            <div class="item">
              <label>MT Lặn</label>
              <div class="sunset">{{moment.unix(data.sys.sunset).format('H:mm')}}</div>
            </div>
          </div>
          <div class="row-two">
            <div class="item">
    ...
    fetchWeather(e) {
      if (e.key == "Enter") {
        fetch(
          ...
          .then(this.setResult, (this.query = ""));
      }
    },
    setResult(results) {
      this.data = results;
